...
static void hubsan_build_packet()
{
    ...
    if(packet_count < 100)
    {
        packet[9] = 0x02 | FLAG_LED | FLAG_FLIP; // sends default value for the 100 first packets (leds on / flips on)
        packet_count++;
    }
    else
    {  
        if(LEDS_ON) { packet[9] = 0x02 | FLAG_LED | FLAG_FLIP; } else { packet[9] = 0x02; } 
    }
